...
function get_type(t::Type{NamedTuple{N, T}}, k::Symbol, default) where {N, T <: Tuple}
    if k in N
...
function unify(a::Type{NamedTuple{A, T}}, b::Type{NamedTuple{B, S}}) where {A,T<:Tuple,B,S<:Tuple}
    c = Dict()
    for (k,v) in zip(A, T.types)
        c[k] = unify(v, get_type(b, k, Nothing))
    end
